...
from Text import zero_padding, get_text_embeddings
...
class Data2Seq(nn.Module):
    ...
    def __init__(self, modality, dim):
        ...
        if (
            self.modality == "image"
            or self.modality == "infrared"
            or self.modality == "x-ray"
        ):
            self.embed = Image.PatchEmbed(embed_dim=self.embed_dim)
        elif self.modality == "text":
            self.embed = get_text_embeddings
        elif self.modality == "video":
            self.embed = Video.PatchEmbed(embed_dim=self.embed_dim)
        elif self.modality == "graph":
            self.embed = Graph.GraphFeatureTokenizer(
                rand_node_id_dim=self.embed_dim, orf_node_id_dim=self.embed_dim
            )
